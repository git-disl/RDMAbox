...
static void do_init(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	...
	size_t bufsize = fuse_chan_bufsize(req->ch);
        ...
        openlog ("libfuse", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);
	...
	//	f->conn.max_write = bufsize;
        //f->conn.max_write = 131072; //128KB
        f->conn.max_write = 1048576; //1MB
        //f->conn.max_write = 4194304; //4MB
	...
	syslog(LOG_NOTICE, "   INIT: %u.%u\n", outarg.major, outarg.minor);
	syslog(LOG_NOTICE, "   flags=0x%08x\n", outarg.flags);
	syslog(LOG_NOTICE, "   max_readahead=0x%08x\n",
			outarg.max_readahead);
        syslog(LOG_NOTICE, "   max_write=0x%08x\n", outarg.max_write);
	syslog(LOG_NOTICE, "   max_background=%i\n",
			outarg.max_background);
	syslog(LOG_NOTICE, "   congestion_threshold=%i\n",
